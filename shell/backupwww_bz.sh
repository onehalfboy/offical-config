...
mkdir -p "$dGoPath"
cd "$configPath"
ls "$configPath" | grep -v .zip | xargs -i tar zcvf "${dConfigPath}/{}.tar.bz" "{}"
cd "$dbPath"
ls "$dbPath" | grep -v .zip | xargs -i tar zcvf "${dDbPath}/{}.tar.bz" "{}"
cd "$webPath"
ls "$webPath" | grep -v .zip | xargs -i tar zcvf "${dWebPath}/{}.tar.bz" "{}"
cd "$goPath"
ls "$goPath" | grep -v .zip | xargs -i tar zcvf "${dGoPath}/{}.tar.bz" "{}"
